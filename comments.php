...
$obt_total_comments = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->comments WHERE comment_post_ID = '$post->ID' AND comment_approved = '1' AND comment_type = ''");
$obt_total_pending_comments = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->comments WHERE comment_post_ID = '$post->ID' AND comment_approved = '0' AND comment_type = ''");
$obt_comments_order = "newer";


	IF ($obt_comments_order == "newer") 
		include(TEMPLATEPATH."/comments-form.php");
	...
	IF ($obt_total_comments == 1){
		echo obt_translate("1 comment");	
	}
	ELSEIF ($obt_total_comments > 1){
		echo obt_translate("%1 comments",$obt_total_comments);
	}
		ELSE{
			IF ($post->comment_status == "open"){
				echo obt_translate("No comments");
			}
			ELSE{
				echo obt_translate("Comments disabled");
			}	
		};
	...
	IF (strlen($post->post_password) && $_COOKIE["wp-postpass_".COOKIEHASH] != $post->post_password){
		...
		$obt_comment_count = 0;
		$obt_can_moderate_comments = 0;
		$obt_comments = $wpdb->get_results("SELECT * FROM $wpdb->comments WHERE comment_post_ID = '$post->ID' AND comment_type = '' AND comment_approved = '1' ORDER BY comment_ID DESC");
		...
		IF ($post->comment_status == "open"){
			echo "<p>".obt_translate("Sé el primero en escribir un comentario").".</p>\n";
		}
		ELSE{
